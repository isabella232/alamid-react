...
Condition.prototype.api = api = {
    ...
    get changes() {
        ...
        condition.query = evaluations.notEquals;
        if (condition.not) {
    ...
    contains: function (value) {
        ...
        condition.addDependency("comparison value", value);
        if (condition.not) {
            condition.query = evaluations.notContains;
        } else {
...
function exists() {
    ...
    if (condition.not) {
        condition.query = evaluations.notExits;
    } else {
...
evaluations.notExits = getNegation(evaluations.exists);
...
evaluations.notEquals = getNegation(evaluations.equals);
...
evaluations.notContains = getNegation(evaluations.notContains);
